...
import { ArcadeApp, type ApprovalController } from "./app";
...
export function startTui(opts: StartTuiOpts): { prompter: ApprovalPrompter; stop(): void } {
  ...
  const boot = async () => {
    ...
    root.render(createElement(ArcadeApp, { ...opts, approval }));
  };

  void boot().catch((error) => {
    opts.onExit();
    console.error("Arcade TUI failed to start", error);
  });

  return {
    prompter(req: ApprovalRequest) {
      return new Promise<ApprovalDecision>((resolve) => {
        if (pending?.state.request.callId === req.callId) pending = { state: { ...pending.state, request: req }, resolve };
        else pending = { state: { request: req, reason: "", mode: "menu" }, resolve };
        publishApprovalState();
      });
    },
    stop() {
